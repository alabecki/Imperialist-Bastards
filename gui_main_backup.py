...
def start_main_screen():
	...
	app.addLabel("upgrade_fortification", "Fortification Level: %.2f" % player.fortification, 1, 1, 2)
	...
	app.addButton("Synthetic Dyes", synthetic_dyes, 1, 2, 3)
	app.addButton("Fertilize Soil", fertilize_soil, 2, 2, 3)
	app.addButton("Synthetic Rubber", synthetic_rubber, 3, 2, 3)
	app.addButton("Synthetic Oil", synthetic_oil, 4, 2, 3)
...
def update_military_gui():
	...
	if len(player.CB) == 0:
		ID = uniform(2, 3)
		app.addLabel("no CB" + ID, "You do not currently have a CB on any nation")
	else:
		count = 1
		for cb in player.CB:
			p = cb.province
			...
			app.setImageTooltip(ID, cb.owner)
			app.addLabel(ID, "%s %d" % (cb.province, cb.time), count, 2)
			count += 1
	app.stopLabelFrame()
...
def gui_load_game(btn):
	...
	global players, relations, provinces, market, auto_save
	auto_save = 
	players = initial["players"]
	relations = initial["relations"]
	provinces = initial["provinces"]
