...
from BalanceScenario.Scenario_provinces import*
...
#	player.technologies.add("high_pressure_steam_engine")
	...
	if type(player) == AI:
		...
		player.mid_class_priority["artists"] += 0.2
...
def hyle(player, provinces):
	...
	if type(player) == AI:
		player.mid_class_priority["officers"] += 0.1
...
def trope(player, provinces):
	...
	if type(player) == AI:
		player.mid_class_priority["bureaucrats"] += 0.2
...
def sidero(player, provinces):
	...
	if type(player) == AI:
		player.mid_class_priority["officers"] += 0.2
...
def isorropia(player, provinces):
	...
	if type(player) == AI:
		player.mid_class_priority["managers"] += 0.2
...
def karbouno(player, provinces): #major
	...
	if type(player) == AI:
		player.mid_class_priority["researchers"] += 0.2
